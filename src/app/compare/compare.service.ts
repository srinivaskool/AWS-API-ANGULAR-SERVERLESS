...
export class CompareService {
  ...
  onStoreData(data: CompareData) {
    ...
    this.authService.getAuthenticatedUser().getSession((err, session) => {
      ...
      this.http.post('https://gy5648at76.execute-api.us-east-2.amazonaws.com/dev/compare-yourself', data, {
        headers: new Headers({'Authorization': session.getIdToken().getJwtToken(), 'Access-Control-Allow-Origin' : '*'})
      })
        .subscribe(
          (result) => {
            this.dataLoadFailed.next(false);
